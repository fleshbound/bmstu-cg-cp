...
std::shared_ptr<KDtree::KDnode> KDtree::_create_tree(std::vector<std::shared_ptr<Object>> objects, const int depth)
{
    ...
    node->bbox = objects[0]->get_bbox();
    QVector3D avg_center(0.0f, 0.0f, 0.0f);

    qInfo( "expanding bboxes" );

    for (auto& object : objects)
    {
        avg_center += object->get_center();
        node->bbox.expand(object->get_bbox());
    }

    avg_center /= objects.size();
    ...
    const QVector3D bbox_size = node->bbox.get_max() - node->bbox.get_min();
    size_t tmp_axis;

    if (bbox_size.x() >= bbox_size.z() && bbox_size.x() >= bbox_size.y())
        tmp_axis = 0;
    else if (bbox_size.y() >= bbox_size.z() && bbox_size.y() >= bbox_size.x())
        tmp_axis = 1;
    else
        tmp_axis = 2;
    ...
    for (auto& object : objects)
    {
        if (object->get_center()[node->axis] < avg_center[node->axis])
